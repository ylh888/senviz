/* rcclient.js : 2020-11-13 Fri : stripped down version of senviz.js/rcsupport.js
2020-12-09 Wed
2020-11-15 Sun  + added CONNECT_HUB (legacy term PASS_THROUGH where data is not interpreted; here PASS_THROUGH=false as we do need to show the sensor data
...
var menu = [
	...
	C_HEIGHT = 200, // for client portal we changed from 660 to 200
	C_RIGHT = 700,
...
function setup() {
	...
	//2020-12-09 Wed   no need as we are using p5.serialport     serialio.web_socket();
	remoteio.web_socket();
	...
	*/
	...
	if (CONNECT_HUB) menu = passthroughMenu

	let i = 0;
	for (let m of menu) { // new menu
		m.b = mk_meter(20, 75 + i * 37, null);
		m.b.text = m.display;
	...
	if (CONNECT_HUB) {
		profileArea.size(1, 1)
	}

	if (DEV) {
		ipArea = createInput(SERVER_ADDR);
		ipArea.position(105, C_HEIGHT - 118);
		ipArea.size(95, 20);

		// portArea = createInput(SERVER_PORT);
		portArea = createInput(31007);

		portArea.position(205, C_HEIGHT - 118);
		portArea.size(45, 20);
	}
	remote_fn();
...
function draw() {
	background('rgb(250,250,250)'); // make the screen greyf
	fill(BASETEXTCOLOUR);
	strokeWeight(2);
	textAlign(LEFT);
	textSize(16);
	if (msgFromServer.length > 0) text(msgFromServer, 260, C_HEIGHT - 105)
	text("" + (remoteio.connected() ? "connected" : " NOT connected"), 260, C_HEIGHT - 108);

	text("USB serial: " + serialio.thisport() + (serialio.connected() ? " connected" : " NOT connected"), 28, C_HEIGHT - 10);

	text(versionText, 28, C_HEIGHT);

	textSize(18);
	text("ACCESS CODE: " + accessCode, 24, C_HEIGHT - 170)
...
function remote_fn() {
	if(DEV) {  // allows changes if in DEV mode
